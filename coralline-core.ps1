# coralline-core.ps1 - shared render engine, dot-sourced by both the daemon
# and the client (as a cold-start fallback when the daemon is unreachable).
...
function Initialize-CorallineRuntime {
...
function Invoke-StatuslineRequest {
    param([string]$RawInputParam, [bool]$SubagentModeParam)
    $rawInput = $RawInputParam
    $SubagentMode = $SubagentModeParam
