...
`VL_LIMIT_SYNC=1` 會讓 `limit5h`／`limit7d` 顯示「你任一 session 看過的最新額度值」，而不是只看當前 session 自己的快照。每次 render 會把 `5h`／`7d` 的值寫進一個每台主機共用的檔（`burn-5h.tsv`、`limit-7d.tsv`），區段則顯示當前視窗中記錄到的最大百分比。預設關閉。
