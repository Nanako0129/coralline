...
倒數會依「相對於視窗重置的急迫度」上色：會在重置前耗盡時變紅，接近邊緣（剛好趕得上
重置）時變黃，能從容趕在重置前的話則轉綠。只有當推算的耗盡時間**比該限額的整個視窗
還長**（5h 或 7d）——代表照這個速度，就算從全新視窗開始也燒不完一輪——數字才會消失、
收斂成亮綠的 `↗ ✓`，因為像 `24d15h` 這種數字純屬噪音。暗淡的 `↗ …` 是暖機狀態——冷啟動
還沒有樣本、無從推算（刻意不用綠勾，免得全新安裝看起來一切健康）。暗淡的 `↗ ✓` 則代表
你只是已經停止消耗，手上沒有進行中的負載。
