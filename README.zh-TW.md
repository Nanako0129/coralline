...
> 這是 [Nanako0129/coralline](https://github.com/Nanako0129/coralline) 的 **Athena fork**，
> 針對 Athena 環境新增四個區段：`ip`（釘在最前）、`worktree`（緊接 git）、`agent`（緊接 model）、
> `warn200k`（釘在最後）。預設 `VL_SEGMENTS` 已啟用這四個區段，其餘皆與上游同步。
...
| 區段 | 顯示內容 |
|---|---|
| `ip` | 本機 IP，快取 `VL_IP_TTL` 秒 — _Athena fork_ |
| `dir` | 目前目錄，過長路徑摺疊為 `~/a/…/z` |
| `git` | 分支、已暫存 `+` / 已修改 `!` / 未追蹤 `?`、領先 `⇡` 落後 `⇣` |
| `worktree` | git worktree 名稱 — _Athena fork_ |
| `model` | 目前使用的 Claude 模型 |
| `agent` | 目前 subagent 名稱 — _Athena fork_ |
...
| `stash` | git stash 數量 |
| `warn200k` | 超過 200k token 警示，沿用主題的 hot 警示色 — _Athena fork_ |

量表會隨用量變色：綠色 → 50% 轉黃 → 75% 轉紅（門檻可調）。

## 為什麼很快

statusline 每秒執行一次（`refreshInterval: 1`），所以腳本必須夠便宜：
...
| `VL_SEGMENTS` | `ip dir git worktree model agent ctx limit5h limit7d cost clock warn200k` | 第一行的區段與順序（`auto` 模式下為完整清單） |
| `VL_IP_TTL` | `30` | IP 快取秒數（Athena fork 的 `ip` 區段） |
