...
原生 renderer 下，`install.ps1` 寫入的是 `statusLine.refreshInterval: 2`，不是 `1`：只要下一個 refresh tick 觸發，Claude Code 就會 `abort()` 正在進行中的 statusline render，而原生 PowerShell renderer 一次 render 接近一秒，所以 1 秒的 tick 幾乎每次都會在 render 完成前把它中斷。重新執行 `install.ps1` 會把既有的 `statusLine` 值（不論是 `1`、`5`、或其他任何值）改寫回 `2`，因為 installer 每次都會整個替換那個值。
